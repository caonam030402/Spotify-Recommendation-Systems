...
def recommend_based_on_search_query(data, scaler, features, k, search_query):
    ...
    if len(matching_indices) == 0:
        st.write("No songs found matching the query.")
        return None
...
if search_query:
    k = st.number_input(
        "Number of Recommendations for Search", min_value=1, max_value=20, value=5
    )

    if st.button("Recommend Based on Search Query"):
        recommended_songs_search = recommend_based_on_search_query(
            data, loaded_scaler, features, k, search_query
        )

        if recommended_songs_search is not None:
            st.write("Recommended Songs:")
            st.dataframe(recommended_songs_search)
